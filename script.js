...
function renderDenominations() {
  ...
  function makeRow(value, type, idx) {
    ...
    input.type = "number";
    input.min = "0";
    input.step = "1"; // Ensuring only whole numbers are allowed
    input.value = "0";
    input.setAttribute("aria-label", `${type} ${value} count`);
    input.dataset.denom = value;
    input.dataset.type = type;
    input.dataset.index = idx;
    input.setAttribute("inputmode", "numeric"); // Ensures a numeric keypad for mobile
    input.addEventListener("input", handleInputChange); // Use 'input' event for instant updates
...
// Handle input changes for immediate calculation
...
// Event wiring for mode buttons (notes/coins/both)
segButtons.forEach((btn) => {
  btn.addEventListener("click", () => {
    segButtons.forEach((b) => b.classList.remove("active"));
    btn.classList.add("active");
...
// Event for country select change
countrySelect.addEventListener("change", () => {
  renderDenominations();
  resultArea.hidden = true;
});
